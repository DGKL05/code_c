...
void build(int u,int l,int r){
	...
	build(u*2,l,mid);
	build(u*2+1,mid+1,r);
	t[u].val=(ll)(t[u*2].val+t[u*2+1].val);
}
...
void pushdown(int u) {
	...
	t[u*2].val+=(ll)(1LL*t[u].lazy*(t[u*2].r-t[u*2].l+1));
	t[u*2+1].val+=(ll)(1LL*t[u].lazy*(t[u*2+1].r-t[u*2+1].l+1));
	// 清除该节点的lazy标记，防止重复更新
	t[u].lazy=0; 
}
...
// 区间修改
void modify2(int u,int l,int r,ll x){ // x为要求区间修改的值
	// 找到子区间，则不需要向下寻找
	if(l<=t[u].l && t[u].r<=r){
		t[u].lazy+=x;
		t[u].val+=1LL*x*(t[u].r-t[u].l+1);
		return;
	}
